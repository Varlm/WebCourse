...
const groups = ref([]); 
const artifacts = ref([]);
const artifactToAdd = ref({});
const userInfoStore=useUserInfoStore();
const artifactToEdit = ref({});
const artifactsPictureRef = ref();
const artifactAddImageUrl=ref()
const imagePreviewUrl = ref(null);
const imagePreviewTitle = ref('');
const artifactEditPictureRef = ref();
const artifactEditImageUrl = ref();
const users = ref([])
const selectedUser = ref(null)
const stats = ref({}) 

function artifactEditPictureChange() {
  if (artifactEditPictureRef.value?.files?.[0]) {
    artifactEditImageUrl.value = URL.createObjectURL(artifactEditPictureRef.value.files[0]);
  } else {
    artifactEditImageUrl.value = null;
  }
}
...
async function fetchArtifacts() {
  ...
  const r = await axios.get('/api/artifacts/', { params });
...
async function onArtifactAdd() {
  const formData = new FormData();

  formData.append('name', artifactToAdd.value.name);
  formData.append(
    'group_id',
    Number(artifactToAdd.value.group_id)
  );

  if (artifactsPictureRef.value?.files?.[0]) {
    formData.append('picture', artifactsPictureRef.value.files[0]);
  }
...
async function onRemoveClick(artifact) {
  await axios.delete(`/api/artifacts/${artifact.id}/`);
  await fetchArtifacts(); 
}

async function onArtifactEditClick(artifact) {
  artifactToEdit.value = {
    id: artifact.id,
    name: artifact.name,
...
async function onUpdateArtifact() {
  const formData = new FormData();

  formData.append('name', artifactToEdit.value.name);
  formData.append(
    'group_id',
    Number(artifactToEdit.value.group_id)
  );

  if (artifactEditPictureRef.value?.files?.[0]) {
    formData.append('picture', artifactEditPictureRef.value.files[0]);
  }

  await axios.put(
    `/api/artifacts/${artifactToEdit.value.id}/`,
    formData,
    {
      headers: {
        'Content-Type': 'multipart/form-data'
...
async function artifactsAddPictureChange(params) {
  artifactAddImageUrl.value=URL.createObjectURL(artifactsPictureRef.value.files[0])
}

function openImagePreview(artifact) {
  imagePreviewUrl.value = artifact.picture;
  imagePreviewTitle.value = artifact.name;
}
